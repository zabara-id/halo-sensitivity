...
из файла ``data/output/L1_DA_opt.csv``.

Параметризация:

    alpha(z) = lower + (upper - lower) / (1 + exp(k * (z - z_mid)))
    alpha2(z) = lower + (upper - lower) / (1 + exp(k * (z - z_mid)))^(1/nu)
    d_max(z) = offset + amplitude * exp(-(z / tau)^p)

* z — высота z0 в тысячах километров (как на графиках).
* В L1_DA_opt ``Deviation Max`` хранится в км, поэтому здесь переводим в тыс. км.
"""
...
DEFAULT_DATA = REPO_ROOT / "data" / "output" / "L1_DA_opt.csv"
...
def logistic5(z: np.ndarray, lower: float, upper: float, k: float, z_mid: float, nu: float) -> np.ndarray:
    """Пятипараметрическая сигмоида для более выраженной горизонтали в хвосте."""
    return lower + (upper - lower) / (1.0 + np.exp(k * (z - z_mid))) ** (1.0 / nu)
...
class FitResult:
    ...
    def predict(self, z: np.ndarray) -> np.ndarray:
        ...
        return logistic5(z, self.lower, self.upper, self.k, self.z_mid, self.nu)
...
def fit_alpha_curve(z: np.ndarray, y: np.ndarray, label: str, use_shape: bool = False) -> FitResult:
    """
    ...
    k_guess = -0.05 if corr > 0 else 0.05
    if use_shape:
        upper_cap = float(np.quantile(y, 0.995)) + 0.0005
        upper_low = max(lower_guess, upper_cap - 0.002)
        upper_high = upper_cap + 0.002
        p0_upper = min(max(upper_guess, upper_low), upper_high)
        p0 = (lower_guess, p0_upper, k_guess, float(np.median(z)), 1.0)
        bounds = (
            (lower_guess - 0.2, upper_low, -5.0, z.min() - 0.2 * span, 0.2),
            (upper_low, upper_high, 5.0, z.max() + 0.2 * span, 10.0),
        )
        try:
            params, _ = curve_fit(
                logistic5,
            ...
            params, _ = curve_fit(logistic5, z, y, p0=p0, maxfev=40000)

        y_hat = logistic5(z, *params)
        return FitResult(
            label=label,
    ...
    p0 = (lower_guess, upper_guess, k_guess, float(np.median(z)))
    bounds = (
...
def print_result(result: FitResult) -> None:
    ...
    if result.nu is None:
        ...
        shape = "асимметричная сигмоида"
    print(f"{result.label}: {direction} {shape}, R^2 = {result.r2:.4f}")
    print(f"    {formula}")


def fit_dmax_curve(z: np.ndarray, dmax_thousand: np.ndarray, label: str) -> DMaxFitResult:
    """
    Подбор растянутой экспоненты offset + A * exp(-(z/tau)^p).
    """
    amplitude_guess = float(dmax_thousand.max() - dmax_thousand.min())
    amplitude_guess = max(amplitude_guess, 1e-3)
    offset_guess = float(np.quantile(dmax_thousand, 0.02))
    ...
    return DMaxFitResult(
        ...
        r2=float(r2_score(dmax_thousand, y_hat)),
    )


def print_dmax_result(result: DMaxFitResult) -> None:
    formula = (
        f"{result.label}(z) = {result.offset:.5f} + {result.amplitude:.5f} "
        f"* exp(-(z/{result.tau:.3f})^{result.power:.3f})"
    )
...
def run_fit_and_plot() -> None:
    """
    Загружает данные L1_DA_opt, строит сигмоиды, печатает параметры и рисует график.
    Без аргументов из консоли.
    """
    z, alpha1, alpha2, dev_max_km = load_alpha_data(DEFAULT_DATA)
    dev_max_thousand = dev_max_km / 1000.0
    fit1 = fit_alpha_curve(z, alpha1, "Alpha1")
    fit2 = fit_alpha_curve(z, alpha2, "Alpha2", use_shape=True)
    fit_dev = fit_dmax_curve(z, dev_max_thousand, "d_max")

    print("Параметризация alpha1: f(z) = lower + (upper - lower) / (1 + exp(k * (z - z_mid)))")
    print("Параметризация alpha2: f(z) = lower + (upper - lower) / (1 + exp(k * (z - z_mid)))^(1/nu)")
    print_result(fit1)
    print_result(fit2)
    print("Параметризация d_max: d(z) = offset + amplitude * exp(-(z/tau)^p)")
